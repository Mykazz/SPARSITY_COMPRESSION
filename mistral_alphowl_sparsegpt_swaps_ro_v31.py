...
V3.1 updates:
  1. Alpha/OWL non-uniform layer allocation.
  2. Row-balanced SparseGPT warm-start masks.
  3. CPU-safe SparseSwaps local mask refinement.
  4. Fixed-mask OBS/SparseGPT reconstruction.
  5. Regional reconstruction:
       after pruning a decoder block, optimize only surviving sparse weights
       to match the original dense block output.
  6. Safer Hessian collection:
       disables autocast inside Hessian matmul.
  7. Sparse-only checkpoint by default.
  8. Atomic checkpoint saving.
  9. --swap_device cpu by default to avoid CUDA indexing asserts.

Recommended safer run:

python mistral_alphowl_sparsegpt_swaps_ro_v31.py \
  --model_id mistralai/Mistral-7B-Instruct-v0.3 \
  --calib data/calib_wikitext103_train_128x2048_mistral.pt \
  --out compressed/test_mlp_s70_alphowl_swaps_ro_v31.pt \
  ...
  --recon_lr 1e-5 \
  --recon_grad_clip 1.0 \
  --activation_clamp 300 \
  --hidden_clamp 5000 \
  --tau 0.18 \
  --mixed_block_matrix \
  --no_save_partial_every_block \
...
def compute_initial_hidden_cache(
    ...
    for i in range(0, n, batch_size):
        ...
        h = emb(input_ids)
        h = sanitize_tensor(h, clamp_abs=hidden_clamp)
...
def run_block_to_cache(
    ...
    for i in range(0, n, batch_size):
        ...
        out = sanitize_tensor(out, clamp_abs=hidden_clamp)
...
class InputSecondMomentCollector:
    def __init__(self, layer: nn.Linear, name: str, activation_clamp: float):
        self.layer = layer
        self.name = name
    ...
    def _hook(self, module: nn.Module, inputs: Tuple[torch.Tensor, ...]) -> None:
        ...
        if not torch.isfinite(x).all() and not self.warned_nonfinite:
...
def compute_owl_outlier_ratio(
    ...
    for r0 in range(0, rows, row_chunk):
        ...
        chunk = sanitize_tensor(W[r0:r1].detach().float(), clamp_abs=1.0e6)
        score = sanitize_tensor(chunk.abs() * col_l2_dev.view(1, -1), clamp_abs=1.0e20)
...
def estimate_pl_alpha_hill(
    ...
    try:
        ...
        Wf = sanitize_tensor(W.detach().to(device=device, dtype=torch.float32), clamp_abs=1.0e6)
        ...
        try:
            ...
            if min_dim <= 2048:
                ...
                if Wf.size(1) >= Wf.size(0):
                    idx = torch.randperm(Wf.size(1), device=Wf.device)[:min(Wf.size(1), 4096)]
                    S = torch.linalg.svdvals(Wf[:, idx])
                else:
                    idx = torch.randperm(Wf.size(0), device=Wf.device)[:min(Wf.size(0), 4096)]
                    S = torch.linalg.svdvals(Wf[idx, :])
...
def collect_allocation_stats_prepass(
    ...
    print("\n" + "=" * 100)
    print("Dense allocation-statistics prepass")
    print("=" * 100)

    for block_idx, block in enumerate(decoder_layers):
        block_prefix = f"{layers_prefix}.{block_idx}"
        infos = block_linear_infos(block, block_prefix, selected_set)

        print("\n" + "-" * 100)
        print(f"[allocation prepass] block {block_idx}/{len(decoder_layers)-1}; selected linears={len(infos)}")

        collectors: Dict[str, InputSecondMomentCollector] = {}

        for full_name, _local_name, layer in infos:
            collectors[full_name] = InputSecondMomentCollector(
                layer=layer,
                name=full_name,
                activation_clamp=activation_clamp,
            )
            collectors[full_name].register()
        ...
        for full_name, _local_name, layer in infos:
            col = collectors[full_name]

            if col.nsamples <= 0:
                print(f"[warn] no activation samples for {full_name}; setting OWL ratio to 0.")
                col_l2 = torch.ones(layer.in_features, dtype=torch.float32)
            else:
                col_l2 = torch.sqrt(col.x2sum.float().clamp_min(1.0e-12))

            print(f"  metric {full_name}: shape={tuple(layer.weight.shape)}")
            t_metric = now()
...
def solve_offset_for_target(
    ...
    if target < min_possible - 1.0e-9 or target > max_possible + 1.0e-9:
        raise ValueError(
            ...
            f"[{min_sparsity}, {max_sparsity}]. Feasible weighted range: "
...
def allocate_nonuniform_sparsities(
    ...
    for b in block_ids:
        alpha_prunable = norm_block_alpha[b]
        owl_prunable = 1.0 - norm_block_owl[b]
        denom = max(alpha_weight + owl_weight, 1.0e-12)
        block_prunability_raw[b] = (
            alpha_weight * alpha_prunable + owl_weight * owl_prunable
        ) / denom
    ...
    layer_alpha_norm_all = {
        s.name: v for s, v in zip(stats, normalize_01([s.alpha_hill for s in stats]))
    }
    layer_owl_norm_all = {
        s.name: v for s, v in zip(stats, normalize_01([s.owl_outlier_ratio for s in stats]))
    }
...
class HessianCollector:
    ...
    Important:
      The hook is called inside the model forward, which may be under autocast.
      Therefore the matmul explicitly disables autocast.
    """
    ...
    def __init__(
        ...
        dtype: torch.dtype,
        name: str,
        activation_clamp: float = 300.0,
        hessian_clamp: float = 1.0e30,
    ):
        self.layer = layer
        ...
        self.name = name
        self.activation_clamp = float(activation_clamp)
        self.hessian_clamp = float(hessian_clamp)
        self.in_features = int(layer.in_features)
        ...
        self.sanitized_hessian_batches = 0
    ...
    def _hook(self, module: nn.Module, inputs: Tuple[torch.Tensor, ...]) -> None:
        ...
        if x.size(-1) != self.in_features:
            print(f"\n[warn] Bad input dim for {self.name}: got {x.size(-1)}, expected {self.in_features}")
            return

        if not torch.isfinite(x).all() and not self.warned_nonfinite_input:
            bad = torch.isfinite(x).logical_not().sum().item()
            total = x.numel()
            print(f"\n[warn] Non-finite activation entering {self.name}: {bad:,}/{total:,}; sanitizing.")
        ...
        if self.hessian_device.type == "cuda":
            with torch.autocast(device_type="cuda", enabled=False):
                x_mat = x.float()
                local_H = 2.0 * x_mat.T.matmul(x_mat)
        else:
            x_mat = x.float() if self.dtype != torch.float64 else x
            local_H = 2.0 * x_mat.T.matmul(x_mat)
        ...
        if not torch.isfinite(local_H).all():
            ...
            if not self.warned_nonfinite_hessian:
                print(
                    ...
                    f"{bad:,}/{total:,}; sanitizing local contribution."
                )
                self.warned_nonfinite_hessian = True

            local_H = sanitize_tensor(local_H, clamp_abs=self.hessian_clamp)
            self.sanitized_hessian_batches += 1

        self.H += local_H.to(dtype=self.dtype)
        self.H = sanitize_tensor(self.H, clamp_abs=self.hessian_clamp)
...
def collect_block_hessians_once(
    block: nn.Module,
    backbone: nn.Module,
    infos: List[Tuple[str, str, nn.Linear]],
    hidden_cache: torch.Tensor,
    batch_size: int,
    main_device: torch.device,
    ...
    for full_name, _local_name, layer in infos:
        ...
        collectors[full_name] = HessianCollector(
            layer=layer,
            hessian_device=hdev,
            dtype=hessian_dtype,
            name=full_name,
    ...
    print(f" Collecting Hessians for {len(infos)} linears...")
    ...
    for name, c in collectors.items():
        if c.sanitized_hessian_batches > 0:
            print(f" [info] {name}: sanitized Hessian batches={c.sanitized_hessian_batches}")
...
def stable_hessian_inverse(
    ...
    H = sanitize_tensor(H, clamp_abs=1.0e30)
    ...
    diag = sanitize_tensor(torch.diag(H_work), clamp_abs=1.0e30)
    H_work[ar, ar] = diag
    ...
    for mult in multipliers:
        ...
        try:
            ...
            Hinv = sanitize_tensor(Hinv, clamp_abs=1.0e30).to(torch.float32)
    ...
    if n <= eig_fallback_max_dim:
        ...
        return sanitize_tensor(Hinv, clamp_abs=1.0e30).to(torch.float32), float(shift)
...
def sparse_swaps_refine_mask(
    ...
    """
    SparseSwaps refinement.

    CPU mode is the default and recommended because CUDA advanced indexing can
    trigger device-side asserts on very large row-wise mask searches.

    For one output row:
        residual r = (1 - m) * w
        local loss L = r^T G r
        c = G r

    Swap p from pruned -> kept and u from kept -> pruned:
        r' = r - w_p e_p + w_u e_u
    ...
    use_cpu = swap_device.lower().strip() in ("cpu", "auto")
    target_device = torch.device("cpu") if use_cpu else original_device

    W_work = sanitize_tensor(
        W.detach().to(device=target_device, dtype=torch.float32),
        clamp_abs=1.0e8,
    ).contiguous()

    G_work = sanitize_tensor(
        G.detach().to(device=target_device, dtype=torch.float32),
        clamp_abs=1.0e30,
    ).contiguous()

    mask_work = mask.detach().to(device=target_device, dtype=torch.bool).contiguous()

    if W_work.shape != mask_work.shape:
        raise ValueError(
            f"SparseSwaps shape mismatch: W={tuple(W_work.shape)}, "
            f"mask={tuple(mask_work.shape)}"
        )

    rows, cols = W_work.shape

    if G_work.shape != (cols, cols):
        raise ValueError(
            f"SparseSwaps Gram shape mismatch: expected {(cols, cols)}, got {tuple(G_work.shape)}"
        )

    candidates = int(max(1, min(candidates, cols)))

    Gdiag = torch.diag(G_work).float().clamp_min(1.0e-12)

    device_label = "CPU-safe" if target_device.type == "cpu" else "CUDA"
    print(
        f"  SparseSwaps {device_label}: computing C = residual_weights @ G, "
        f"rows={rows}, cols={cols}, candidates={candidates}, iters={max_iters}"
    )

    residual_weights = torch.where(mask_work, torch.zeros_like(W_work), W_work)
    C_work = sanitize_tensor(residual_weights.matmul(G_work), clamp_abs=1.0e30)
    ...
    for i in range(rows):
        ...
        for _it in range(max_iters):
            pruned = torch.where(~m)[0].long()
            kept = torch.where(m)[0].long()

            if pruned.numel() == 0 or kept.numel() == 0:
                break

            pruned = pruned[(pruned >= 0) & (pruned < cols)]
            kept = kept[(kept >= 0) & (kept < cols)]
            ...
            remove_delta = (
                -2.0 * wp_all * cp_all
                + (wp_all * wp_all) * Gdiag[pruned]
            )
            remove_delta = sanitize_tensor(remove_delta, clamp_abs=1.0e30)
            ...
            p_idx = p_idx[(p_idx >= 0) & (p_idx < cols)]
            ...
            p_delta = (
                -2.0 * w[p_idx] * c[p_idx]
                + (w[p_idx] * w[p_idx]) * Gdiag[p_idx]
            )
            p_delta = sanitize_tensor(p_delta, clamp_abs=1.0e30)
            wp = w[p_idx]
            ...
            add_delta = (
                2.0 * wu_all * cu_all
                + (wu_all * wu_all) * Gdiag[kept]
            )
            add_delta = sanitize_tensor(add_delta, clamp_abs=1.0e30)
            ...
            u_idx = kept[u_local]
            u_idx = u_idx[(u_idx >= 0) & (u_idx < cols)]
            ...
            u_delta = (
                2.0 * w[u_idx] * c[u_idx]
                + (w[u_idx] * w[u_idx]) * Gdiag[u_idx]
            )
            u_delta = sanitize_tensor(u_delta, clamp_abs=1.0e30)
            wu = w[u_idx]

            G_pu = G_work[p_idx][:, u_idx]
            ...
            delta = sanitize_tensor(delta, clamp_abs=1.0e30)
            ...
            flat_int = int(flat_idx.item())
            p_pos = flat_int // ku
            u_pos = flat_int % ku

            if p_pos < 0 or p_pos >= kp or u_pos < 0 or u_pos >= ku:
                print(
                    f"\n  [warn] invalid SparseSwaps pair: "
                    f"p_pos={p_pos}, kp={kp}, u_pos={u_pos}, ku={ku}. Stopping row."
                )
                break
            ...
            if p < 0 or p >= cols or u < 0 or u >= cols:
                print(f"\n  [warn] invalid SparseSwaps indices p={p}, u={u}, cols={cols}. Stopping row.")
                break

            wp_scalar = float(w[p].item())
            wu_scalar = float(w[u].item())

            m[p] = True
            m[u] = False

            c.add_(G_work[:, u], alpha=wu_scalar)
            c.add_(G_work[:, p], alpha=-wp_scalar)
            c[:] = sanitize_tensor(c, clamp_abs=1.0e30)
        ...
        if row_print_every > 0 and (i + 1) % row_print_every == 0:
            print(
                f"\r  SparseSwaps {device_label} rows {i + 1}/{rows}; "
    ...
    print(
        f"\r  SparseSwaps {device_label} rows {rows}/{rows}; "
        f"accepted={accepted_total:,}; elapsed={fmt_time(now() - t0)}"
    )

    return mask_work.to(device=original_device, dtype=torch.bool)


# =============================================================================
# Fixed-mask OBS reconstruction
# =============================================================================
...
def fixed_mask_obs_reconstruction(
    ...
    W = sanitize_tensor(W.float(), clamp_abs=1.0e8).clone()
    Hinv = sanitize_tensor(Hinv.float(), clamp_abs=1.0e30)
    mask = mask.bool()
    ...
    for i1 in range(0, cols, blocksize):
        ...
        for local_i in range(count):
            ...
            err = sanitize_tensor((w - q) / d, clamp_abs=1.0e8)
            Err1[:, local_i] = err
            ...
            if local_i + 1 < count:
                ...
                W1[:, local_i + 1:count] = sanitize_tensor(
                    W1[:, local_i + 1:count],
                    clamp_abs=1.0e8,
                )
    ...
    return sanitize_tensor(Q * mask.to(Q.dtype), clamp_abs=1.0e8)
...
@torch.no_grad()
def make_diagonal_hessian_fallback(
    layer: nn.Linear,
    device: torch.device,
    dtype: torch.dtype,
    diag_value: float = 1.0,
) -> torch.Tensor:
    cols = int(layer.in_features)
    return torch.eye(cols, device=device, dtype=dtype) * float(diag_value)
...
def compress_linear_alphowl_sparsegpt_swaps(
    ...
    W_orig = sanitize_tensor(W_orig, clamp_abs=1.0e8)
    ...
    if dead.any():
        ...
        print(f"  [warn] {n_dead} dead/near-dead Hessian columns; stabilizing.")
        W_orig[:, dead] = 0.0
    ...
    print(f"  swap_device={swap_device}")
    ...
    Q = sanitize_tensor(Q * mask.to(Q.dtype), clamp_abs=1.0e8)
...
class TrainableSparseLinear(nn.Module):
    def __init__(self, original: nn.Linear, mask_cpu: torch.Tensor, device: torch.device, dtype: torch.dtype):
        super().__init__()

        self.in_features = int(original.in_features)
        self.out_features = int(original.out_features)

        mask = mask_cpu.detach().cpu().bool()
        self.register_buffer("mask", mask.to(device=device, dtype=torch.bool).contiguous())

        w0 = original.weight.detach().to(device=device, dtype=dtype)
        w0 = w0 * self.mask.to(dtype)
    ...
    def clamp_mask(self) -> None:
        ...
        self.weight.data = sanitize_tensor(self.weight.data, clamp_abs=1.0e8)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return F.linear(x, w.to(device=x.device, dtype=x.dtype), bias)


def install_trainable_sparse_layers(
    block: nn.Module,
    infos: List[Tuple[str, str, nn.Linear]],
    masks_cpu: Dict[str, torch.Tensor],
    device: torch.device,
    dtype: torch.dtype,
) -> None:
    for full_name, local_name, _old_layer in infos:
        old = get_module_by_name(block, local_name)

        if not isinstance(old, nn.Linear):
            raise TypeError(f"Expected nn.Linear at {local_name}, got {type(old)}")

        mod = TrainableSparseLinear(
            original=old,
            mask_cpu=masks_cpu[full_name],
            device=device,
            dtype=dtype,
        )
...
@torch.no_grad()
def replace_trainable_sparse_with_dense(
    block: nn.Module,
    infos: List[Tuple[str, str, nn.Linear]],
    dtype: torch.dtype,
    device: torch.device,
) -> None:
    for _full_name, local_name, _old_layer_ref in infos:
        ...
        dense.weight.data.copy_(
            (mod.weight.detach() * mod.mask.to(mod.weight.dtype)).to(device=device, dtype=dtype)
        )

        if mod.bias is not None:
            dense.bias.data.copy_(mod.bias.detach().to(device=device, dtype=dtype))

        set_module_by_name(block, local_name, dense)


def regional_reconstruct_block(
    block: nn.Module,
    backbone: nn.Module,
    infos: List[Tuple[str, str, nn.Linear]],
    masks_cpu: Dict[str, torch.Tensor],
    hidden_cache: torch.Tensor,
    y_dense_cache: torch.Tensor,
    batch_size: int,
    main_device: torch.device,
    model_dtype: torch.dtype,
    steps: int,
    ...
    install_trainable_sparse_layers(
        block=block,
        infos=infos,
        masks_cpu=masks_cpu,
        device=main_device,
    ...
    sparse_mods: List[TrainableSparseLinear] = []

    for _full_name, local_name, _old_layer in infos:
        mod = get_module_by_name(block, local_name)
        if isinstance(mod, TrainableSparseLinear):
            params.append(mod.weight)
            sparse_mods.append(mod)

    if not params:
        print(" [warn] no trainable sparse parameters; skipping RO.")
        return

    optimizer = torch.optim.AdamW(params, lr=lr, weight_decay=weight_decay)

    n = hidden_cache.size(0)
    autocast_enabled = main_device.type == "cuda" and model_dtype in (torch.float16, torch.bfloat16)
    ...
    for step in range(1, steps + 1):
        ...
        for i in range(0, n, batch_size):
            x = hidden_cache[i:i + batch_size].to(main_device, dtype=model_dtype)
            y = y_dense_cache[i:i + batch_size].to(main_device, dtype=model_dtype)

            x = sanitize_tensor(x, clamp_abs=hidden_clamp)
            y = sanitize_tensor(y, clamp_abs=hidden_clamp)

            optimizer.zero_grad(set_to_none=True)

            with torch.autocast(device_type=main_device.type, dtype=model_dtype, enabled=autocast_enabled):
                pred = call_decoder_block(block, x, backbone)

            pred_f = sanitize_tensor(pred.float(), clamp_abs=hidden_clamp)
            y_f = sanitize_tensor(y.float(), clamp_abs=hidden_clamp)

            mse = F.mse_loss(pred_f, y_f)
            denom = torch.mean(y_f * y_f).clamp_min(1.0e-6)
            loss = mse / denom

            if not torch.isfinite(loss):
                print(" [warn] non-finite regional reconstruction loss; skipping batch.")
                optimizer.zero_grad(set_to_none=True)
                continue
        ...
        if step == 1 or step % 5 == 0 or step == steps:
            ...
            print(f"  recon step {step:4d}/{steps} relative_mse={avg:.6e} elapsed={fmt_time(now() - t0)}")

    block.eval()

    replace_trainable_sparse_with_dense(
        block=block,
        infos=infos,
        dtype=model_dtype,
        device=main_device,
...
# =============================================================================
# Sparse state extraction and checkpoints
# =============================================================================

@torch.no_grad()
def extract_sparse_states_from_block(
    block: nn.Module,
    infos: List[Tuple[str, str, nn.Linear]],
    masks_cpu: Dict[str, torch.Tensor],
    target_sparsities: Dict[str, float],
    ...
    for full_name, local_name, _old_ref in infos:
        layer = get_module_by_name(block, local_name)

        if not isinstance(layer, nn.Linear):
            raise TypeError(f"Expected dense nn.Linear after reconstruction at {local_name}, got {type(layer)}")

        mask = masks_cpu[full_name].detach().cpu().bool()
        dense = layer.weight.detach().cpu().float() * mask.to(torch.float32)
        ...
        values = dense[mask].to(dtype=value_dtype).contiguous()
        mask_packed = pack_bool_mask_rows(mask)
        ...
        st: Dict[str, Any] = {
            ...
            "value_dtype": dtype_name(value_dtype),
            "sparsity": float(actual_sparsity),
            "target_sparsity": float(target_sparsities[full_name]),
            "pruned_count": int(pruned),
            "total_count": int(total),
            "method": "alphowl_rowbalanced_sparsegpt_sparseswaps_fixedmask_obs_regional_reconstruction",
        }

        st.update(method_extra)

        if store_debug_dense_weight:
            st["dense_sparse_weight"] = dense.to(torch.float16)
...
def save_checkpoint(
    ...
    sparse_only_checkpoint: bool,
) -> None:
    if keep_dequantized_state_dict:
        model_state = {k: v.detach().cpu() for k, v in model.state_dict().items()}
    elif sparse_only_checkpoint:
        model_state = {}
    else:
        compressed_weight_keys = {f"{name}.weight" for name in sparse_layers.keys()}
        model_state = {
            k: v.detach().cpu()
            for k, v in model.state_dict().items()
            if k not in compressed_weight_keys
        }

    ckpt = {
        "format": "hf_alphowl_sparsegpt_sparseswaps_ro_v31",
        "model_id": model_id,
...
def build_meta(
    ...
    meta: Dict[str, Any] = {
        "method": "alphowl_rowbalanced_sparsegpt_sparseswaps_fixedmask_obs_ro_v31",
        ...
        "total_pruned_selected_weights": int(total_pruned),
        ...
        "swap_device": str(args.swap_device),
        ...
        "activation_clamp": float(args.activation_clamp),
        "hidden_clamp": float(args.hidden_clamp),
        "allow_hessian_diag_fallback": bool(args.allow_hessian_diag_fallback),
        ...
        "sparse_only_checkpoint": bool(args.sparse_only_checkpoint),
...
def compress_model_blockwise(
    ...
    print("\n" + "=" * 100)
    print("Blockwise Alpha/OWL + SparseGPT + SparseSwaps + Regional Reconstruction")
    print("=" * 100)
    ...
    for block_idx, block in enumerate(decoder_layers):
        ...
        print(f"Selected linears: {len(infos)}")
        print(f"Hidden cache entering block: shape={tuple(hidden_cache.shape)} dtype={hidden_cache.dtype}")
        print(f"CUDA memory: {cuda_mem()}")

        for full_name, _local_name, layer in infos:
            print(
                f" - {full_name}: shape={tuple(layer.weight.shape)} "
                f"target_sparsity={100.0 * layer_sparsities[full_name]:.2f}%"
            )
        ...
        print("\n Computing dense block target for regional reconstruction...")
        y_dense_cache = run_block_to_cache(
            block=block,
            backbone=backbone,
            hidden_cache=hidden_cache,
            batch_size=args.batch_size,
            ...
            amp_dtype=model_dtype,
            storage_dtype=hidden_cache_dtype,
            desc=f"block {block_idx} dense target",
            hidden_clamp=float(args.hidden_clamp),
        )
        ...
        masks_cpu: Dict[str, torch.Tensor] = {}
        method_extra_per_layer: Dict[str, Dict[str, Any]] = {}

        for local_idx, (full_name, _local_name, layer) in enumerate(infos, start=1):
            print("\n" + "-" * 100)
            print(f"[{local_idx}/{len(infos)}] Compressing {full_name}")
            print(f" shape={tuple(layer.weight.shape)} dtype={layer.weight.dtype}")

            H, nsamples, hdev = hessians[full_name]
            print(f" H shape={tuple(H.shape)} samples={nsamples} device={hdev}")

            if nsamples <= 0:
                if bool(args.allow_hessian_diag_fallback):
                    print(
                        f" [warn] No Hessian samples for {full_name}. "
                        f"Using diagonal fallback H = I."
                    )
                    H = make_diagonal_hessian_fallback(
                        layer=layer,
                        device=hdev,
                        dtype=hessian_dtype,
                        diag_value=1.0,
                    ...
                    raise RuntimeError(f"No Hessian samples collected for {full_name}.")
            ...
            result = compress_linear_alphowl_sparsegpt_swaps(
                ...
                H=H,
                nsamples=nsamples,
                target_sparsity=float(layer_sparsities[full_name]),
                percdamp=float(args.percdamp),
                blocksize=int(args.blocksize),
                mask_score=str(args.mask_score),
                act_order=bool(args.act_order),
            ...
            masks_cpu[full_name] = result.mask.cpu().bool()
            method_extra_per_layer[full_name] = {
                ...
                "regional_reconstruction_steps": int(args.recon_steps),
            }

            print(
                f" compressed layer: actual_sparsity={100.0 * result.sparsity:.2f}% "
                f"time={fmt_time(now() - t_layer)}"
            )

            del H
            del hessians[full_name]
            del result
            cleanup()

        if int(args.recon_steps) > 0:
            with torch.enable_grad():
                regional_reconstruct_block(
                    block=block,
                    backbone=backbone,
                    infos=infos,
                    masks_cpu=masks_cpu,
                    hidden_cache=hidden_cache,
                    y_dense_cache=y_dense_cache,
                    batch_size=int(args.batch_size),
                    main_device=main_device,
                    model_dtype=model_dtype,
                    steps=int(args.recon_steps),
                    lr=float(args.recon_lr),
                    weight_decay=float(args.recon_weight_decay),
                    grad_clip=float(args.recon_grad_clip),
                    hidden_clamp=float(args.hidden_clamp),
                )

        method_extra_common = {
            "regional_reconstruction_steps": int(args.recon_steps),
            "regional_reconstruction_lr": float(args.recon_lr),
            "regional_reconstruction_weight_decay": float(args.recon_weight_decay),
            "regional_reconstruction_grad_clip": float(args.recon_grad_clip),
        }

        block_states = extract_sparse_states_from_block(
            block=block,
            infos=infos,
            masks_cpu=masks_cpu,
            target_sparsities=layer_sparsities,
            value_dtype=value_dtype,
            store_debug_dense_weight=bool(args.store_debug_dense_weight),
            method_extra=method_extra_common,
        )

        for name, st in block_states.items():
            st.update(method_extra_per_layer.get(name, {}))
            sparse_layers[name] = st
            print(
                f" saved {name}: actual_sparsity={100.0 * st['sparsity']:.2f}% "
                f"values={tuple(st['values'].shape)} mask={tuple(st['mask'].shape)}"
            )

        print("\n Running compressed/reconstructed block to create next hidden cache...")
        ...
        if args.save_partial_every_block:
            ...
            meta_partial = build_meta(
                args,
                sparse_layers,
                layer_sparsities,
                script_t0,
                partial=True,
                completed_block=block_idx,
            )
...
def main() -> None:
    ...
    parser.add_argument("--activation_clamp", type=float, default=300.0)
    parser.add_argument("--hidden_clamp", type=float, default=5000.0)
    parser.add_argument("--allow_hessian_diag_fallback", action="store_true", default=True)
    parser.add_argument("--no_allow_hessian_diag_fallback", dest="allow_hessian_diag_fallback", action="store_false")

    parser.add_argument("--swap_iters", type=int, default=5)
    parser.add_argument("--swap_candidates", type=int, default=32)
    ...
    parser.add_argument(
        ...
        help="Use cpu for SparseSwaps to avoid CUDA advanced-indexing device-side asserts.",
    )

    parser.add_argument("--recon_steps", type=int, default=20)
    parser.add_argument("--recon_lr", type=float, default=1.0e-5)
    ...
    parser.add_argument("--save_partial_every_block", action="store_true", default=False)
    parser.add_argument("--no_save_partial_every_block", dest="save_partial_every_block", action="store_false")
    parser.add_argument("--sparse_only_checkpoint", action="store_true", default=True)
    parser.add_argument("--no_sparse_only_checkpoint", dest="sparse_only_checkpoint", action="store_false")
    ...
    if args.swap_device == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("--swap_device cuda requested but CUDA is unavailable.")
    ...
    if args.alpha_device == "cuda" and torch.cuda.is_available():
        alpha_device = torch.device("cuda")
    else:
        alpha_device = torch.device("cpu")

    if torch.cuda.is_available():
        torch.cuda.reset_peak_memory_stats()

    print("=" * 100)
    print("Mistral Alpha/OWL + SparseGPT + SparseSwaps + Regional Reconstruction V3.1")
    print("=" * 100)
    ...
    print(f"activation_clamp: {args.activation_clamp}")
    print(f"hidden_clamp: {args.hidden_clamp}")
    print(f"swap_iters: {args.swap_iters}")
    print(f"swap_candidates: {args.swap_candidates}")
    print(f"swap_device: {args.swap_device}")
    print(f"recon_steps: {args.recon_steps}")
    print(f"recon_lr: {args.recon_lr}")
    print(f"sparse_only_checkpoint: {args.sparse_only_checkpoint}")
    print(f"CUDA memory: {cuda_mem()}")
