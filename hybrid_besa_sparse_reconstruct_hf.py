#!/usr/bin/env python3
"""
Hybrid BESA / SparseGPT / Wanda++-style sparse-only compression for Hugging Face decoder-only LMs.

Primary target:
    High global sparsity, for example 70%, with less perplexity damage than
    uniform sparse + INT4.

Designed for:
    - mistralai/Mistral-7B-Instruct-v0.3
    - Mistral/LLaMA/Qwen-style decoder-only AutoModelForCausalLM models with nn.Linear projections

Mathematical pipeline per transformer block:

    1. Cache block input hidden states X_block.
    2. Compute dense block output Y_dense = Block_dense(X_block).
    3. Collect full second-order Hessian proxies for every selected Linear layer:

           H_l = 2 X_l^T X_l

       where X_l is the actual input activation to that Linear layer.

    4. Generate several BESA-style non-uniform layer sparsity allocations under a
       global block sparsity target. For Mistral 70%, this protects sensitive layers
       and pushes more pruning into gate/up projections.

    5. For each allocation candidate:
          - Restore original block weights.
          - SparseGPT-prune each Linear layer with Hessian error compensation.
          - Run the compressed block.
          - Score block reconstruction error:

                ||Y_dense - Y_sparse||_2^2 / ||Y_dense||_2^2

    6. Pick the candidate with the smallest block error.
    7. Re-apply the best candidate permanently.

    8. Optional Wanda++/BESA-style regional reconstruction:
          Keep the binary masks fixed and optimize only surviving dense weights:

              min_{W_kept} || Block_sparse(X_block; M \odot W_kept) - Y_dense ||_2^2

       Pruned entries are forced to zero after every optimizer step.

    9. Run the final sparse block to create the next block's input cache.

Storage format:
    ckpt["hybrid_sparse_layers"][layer_name] = {
        "shape": [out_features, in_features],
        "mask": packed bit mask,
...
Important:
    This script stores only the surviving values, not a full dense sparse matrix.
    It is still not an optimized sparse-kernel deployment format; evaluation wrappers
    reconstruct dense weights before F.linear.

Safer changes in this version:
    - Stores kept values as 1D tensors, matching eval_universal.py SparseValueLinear.
    - Adds --resume_from_partial and --save_partial_every_block.
    - Saves a partial checkpoint after each completed block by default.
    - Can skip completed layers from a partial checkpoint.
    - Forces hooks to be removed via try/finally.
    - Adds more NaN/Inf guards and CUDA cleanup.
    - Adds per-block emergency partial save on exception.
    - Adds safer defaults for 70%: CPU Hessian for large layers, conservative LR, grad clipping.
"""
...
def make_allocation_candidates(
    ...
    templates.append(template({
        ...
        "v_proj": target_sparsity - 0.20,
        "o_proj": target_sparsity - 0.35,
        "gate_proj": target_sparsity + 0.18,
        "up_proj": target_sparsity + 0.18,
        "down_proj": target_sparsity - 0.18,
    }))
    templates.append(template({
        "q_proj": target_sparsity - 0.18,
        "k_proj": target_sparsity - 0.18,
        "v_proj": target_sparsity - 0.25,
        "o_proj": target_sparsity - 0.38,
        "gate_proj": target_sparsity + 0.23,
        "up_proj": target_sparsity + 0.23,
        "down_proj": target_sparsity - 0.12,
    }))
    templates.append(template({
        "q_proj": target_sparsity - 0.05,
        "k_proj": target_sparsity - 0.05,
        "v_proj": target_sparsity - 0.20,
        "o_proj": target_sparsity - 0.30,
        "gate_proj": target_sparsity + 0.17,
        "up_proj": target_sparsity + 0.17,
        "down_proj": target_sparsity - 0.30,
    }))
    templates.append(template({
        "q_proj": target_sparsity - 0.02,
        "k_proj": target_sparsity - 0.02,
        "v_proj": target_sparsity - 0.28,
        "o_proj": target_sparsity - 0.35,
        "gate_proj": target_sparsity + 0.16,
        "up_proj": target_sparsity + 0.16,
        "down_proj": target_sparsity - 0.25,
    }))
    templates.append(template({
        "q_proj": target_sparsity - 0.08,
        "k_proj": target_sparsity - 0.08,
        "v_proj": target_sparsity - 0.12,
        "o_proj": target_sparsity - 0.22,
...
def dense_weight_from_sparse_state(state: Dict[str, Any], device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    ...
    values = state["values"].to(device=device, dtype=dtype)
    mask = unpack_bool_mask_rows(state["mask"].to(device), original_cols=cols)

    if values.ndim == 2:
        # Backward compatibility with old dense-values checkpoints.
        if tuple(values.shape) != (rows, cols):
            raise ValueError(f"Bad dense values shape {tuple(values.shape)} expected {(rows, cols)}")
        w = values * mask.to(values.dtype)
        return sanitize_tensor(w)

    n_kept = int(mask.sum().item())
    if values.numel() != n_kept:
        raise ValueError(
            f"Kept values count mismatch: values={values.numel():,}, mask_kept={n_kept:,}, shape={(rows, cols)}"
        )

    w = torch.zeros((rows, cols), device=device, dtype=dtype)
    w[mask] = values
    return sanitize_tensor(w)
...
def install_trainable_sparse_layers(
    ...
    for full_name, local_name, old_layer in layer_infos:
        ...
        mask = unpack_bool_mask_rows(st["mask"].to(device), original_cols=cols)

        tmp = nn.Linear(cols, rows, bias=old_layer.bias is not None, device=device, dtype=dtype)
        tmp.weight.data.copy_(dense_weight_from_sparse_state(st, device=device, dtype=dtype))
        if old_layer.bias is not None:
            tmp.bias.data.copy_(old_layer.bias.detach().to(device=device, dtype=dtype))

        mod = TrainableSparseLinear(tmp, mask=mask, device=device, dtype=dtype)
...
def extract_states_from_trainable_sparse(
    ...
    for full_name, local_name, _old_layer in layer_infos:
        ...
        mod.clamp_mask()
        mask = mod.mask.detach().cpu().bool()
        dense = mod.weight.detach().cpu()
        states[full_name]["values"] = dense[mask].to(dtype=value_dtype).contiguous()
...
def replace_trainable_sparse_with_dense_linear(
    ...
    for full_name, local_name, old_layer in layer_infos_original:
        ...
        dense.weight.data.copy_(dense_weight_from_sparse_state(st, device=device, dtype=dtype))
        if old_layer.bias is not None:
...
def build_meta(
    ...
    return {
        "method": "hybrid_besa_sparsegpt_wandapp_sparse_only_surviving_weight_reconstruction_safe",
        ...
        "note": (
            "Sparse-only high-sparsity experiment. Uses BESA-style block allocation search, "
            "SparseGPT pruning with Hessian compensation, and optional Wanda++/BESA-style "
            "regional reconstruction over surviving weights. No INT4 quantization is applied. "
            "This safe version stores only 1D kept values in mask order."
        ),
    }
...
def compress_model_blockwise(
    ...
    # If resuming, replay completed blocks by applying saved sparse weights and forwarding.
    ...
    if start_block > 0:
        ...
        for bi in range(start_block):
            ...
            if len(block_states) != len(layer_infos):
                raise RuntimeError(
                    f"Cannot resume: block {bi} expected {len(layer_infos)} layer states but found {len(block_states)}."
                )
            apply_dense_weights_from_states(layer_infos, block_states, main_device)
            hidden_cpu = run_block_on_hidden_cache(
                base, block, hidden_cpu, batch_size, main_device, model_dtype, hidden_cache_dtype,
                desc=f"resume block {bi} sparse forward",
            )
            cleanup()
    ...
    for bi, block in enumerate(decoder_layers):
        ...
        try:
            if not layer_infos:
                ...
                hidden_cpu = run_block_on_hidden_cache(
                    base, block, hidden_cpu, batch_size, main_device, model_dtype, hidden_cache_dtype,
                    f"block {bi} dense forward",
                )
                continue

            print("\n  Computing dense block output target Y_dense...")
            y_dense_cpu = run_block_on_hidden_cache(
                base, block, hidden_cpu, batch_size, main_device, model_dtype, hidden_cache_dtype,
                f"block {bi} dense target",
            )
            ...
            for ci, rates in enumerate(candidates, start=1):
                ...
                y_comp_cpu = run_block_on_hidden_cache(
                    base, block, hidden_cpu, batch_size, main_device, model_dtype, hidden_cache_dtype,
                    f"candidate {ci} sparse block",
                )
            ...
            for full_name, _local_name, _layer in layer_infos:
                ...
                if store_debug_dense_weight:
                    st["dense_debug_weight"] = dense_weight_from_sparse_state(st, device=main_device, dtype=torch.float16).cpu()
                sparse_layers[full_name] = st

            print("\n  Running final sparse block to create next hidden cache...")
            hidden_cpu = run_block_on_hidden_cache(
                base, block, hidden_cpu, batch_size, main_device, model_dtype, hidden_cache_dtype,
                f"block {bi} final sparse forward",
            )
            ...
            if args.save_partial_every_block:
                save_partial_checkpoint(model, tokenizer, args, sparse_layers, suffixes, caps, script_t0, completed_block=bi)
            ...
            traceback.print_exc()
            if sparse_layers:
                print("[error] Saving emergency partial checkpoint before re-raising...")
                save_partial_checkpoint(model, tokenizer, args, sparse_layers, suffixes, caps, script_t0, completed_block=bi - 1)
            raise
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional cap override, e.g. "
            "q_proj:0.35-0.60,k_proj:0.35-0.60,v_proj:0.25-0.55,o_proj:0.15-0.45,"
            "gate_proj:0.70-0.92,up_proj:0.70-0.92,down_proj:0.35-0.68"
        ),
    )
    ...
    print("=" * 100)
    print("Hybrid BESA / SparseGPT / Wanda++ sparse-only compression SAFE")
    print("=" * 100)
