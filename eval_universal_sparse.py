...
1. Original HF model:
    - FP16/BF16/FP32
    - NF4 4-bit BitsAndBytes baseline

2. Older GPTQ / SparseGPT custom checkpoints:
    - ckpt["layers"]
    - ckpt["joint_sparsegpt_gptq_layers"]

3. New Alpha/OWL + SparseGPT + SparseSwaps sparse-only checkpoints:
    - ckpt["alphowl_sparse_layers"]

This evaluator reconstructs the new sparse-only layers like this:

    mask = unpack packedbits mask
    values = kept 1D row-major values
    W_sparse = zeros(shape)
    W_sparse[mask] = values

Then it writes W_sparse into the corresponding HF nn.Linear layer.
...
Example for new sparse-only checkpoint:

python eval_universal_sparse.py \
  --input_file data/eval_wikitext103_test_shared_250k.txt \
  --model_id mistralai/Mistral-7B-Instruct-v0.3 \
  --compressed compressed/test_mlp_s70_alphowl_swaps_sparseonly.pt \
  --load_mode fp \
  --dtype float16 \
  --block_size 1024 \
  --batch_size 1 \
  --attn_implementation eager \
  --save_json results/test_mlp_s70_alphowl_swaps_eval.json
"""
...
def unpack_bool_mask_rows(packed: torch.Tensor, original_cols: int) -> torch.Tensor:
    """
    Unpack packed sparse bool mask.
    ...
    return mask[:, :original_cols].contiguous()
...
# =============================================================================
# Runtime layer for old quantized sparse checkpoints
# =============================================================================

class SparseQuantLinear(nn.Module):
    """
    Runtime wrapper for older compressed SparseGPT+GPTQ linear layers.
...
# =============================================================================
# New sparse-only reconstruction helpers
# =============================================================================

@torch.no_grad()
def reconstruct_dense_weight_from_alphowl_state_cpu(
    st: Dict[str, Any],
    dtype: torch.dtype,
) -> torch.Tensor:
    """
    Reconstructs the new sparse-only layer format on CPU.

    Expected new format:
        st["shape"] = [out_features, in_features]
        st["mask"]  = packedbits or bool mask
        st["values"] = 1D kept values in row-major mask order
        st["values_format"] = "kept_1d_row_major"

    Output:
        dense zero-masked weight [out_features, in_features]
    """
    rows, cols = tuple(st["shape"])
    shape = (int(rows), int(cols))

    mask_packing = str(st.get("mask_packing", "packedbits"))
    mask_stored = st.get("mask", None)

    if mask_stored is None:
        raise ValueError("alphowl sparse layer state has no mask.")

    if mask_packing == "packedbits":
        mask = unpack_bool_mask_rows(mask_stored.cpu().to(torch.uint8), original_cols=shape[1])
    elif mask_packing == "bool":
        mask = mask_stored.cpu().bool()
    else:
        raise ValueError(f"Unsupported alphowl mask_packing: {mask_packing}")

    if tuple(mask.shape) != shape:
        raise ValueError(f"Mask shape mismatch: expected {shape}, got {tuple(mask.shape)}")

    if "dense_sparse_weight" in st:
        w = st["dense_sparse_weight"].detach().cpu().to(dtype=dtype)
        if tuple(w.shape) != shape:
            raise ValueError(f"dense_sparse_weight shape mismatch: expected {shape}, got {tuple(w.shape)}")
        return w * mask.to(dtype=w.dtype)

    values = st.get("values", None)
    if values is None:
        raise ValueError("alphowl sparse layer state has neither 'values' nor 'dense_sparse_weight'.")

    values = values.detach().cpu().to(dtype=dtype).contiguous()

    n_kept = int(mask.sum().item())
    if values.numel() != n_kept:
        raise ValueError(
            f"Kept values count mismatch: values={values.numel():,}, "
            f"mask_kept={n_kept:,}, shape={shape}"
        )

    # >>> CRUCIAL PART:
    # Row-major boolean assignment must happen on CPU to avoid CUDA indexing issues.
    w = torch.zeros(shape, dtype=dtype, device="cpu")
    w[mask] = values
...
@torch.no_grad()
def apply_alphowl_sparse_checkpoint(
    model: nn.Module,
    ckpt: Dict[str, Any],
    dtype: torch.dtype,
    ...
    """
    Applies ckpt["alphowl_sparse_layers"] to an already loaded base HF model.

    This does not wrap layers. It directly writes reconstructed dense-zero weights
    into existing nn.Linear weights, which makes evaluation simple and fast.
    """
    layers = ckpt.get("alphowl_sparse_layers", None)
    if not isinstance(layers, dict) or len(layers) == 0:
        raise ValueError("Checkpoint does not contain a valid ckpt['alphowl_sparse_layers'].")

    items = list(layers.items())
    print(f"Applying Alpha/OWL sparse-only layers: {len(items):,}")
    print(f"Reconstruction dtype: {dtype}")

    iterator = items
    if show_progress and tqdm is not None:
        iterator = tqdm(items, desc="Applying alphowl sparse layers", unit="layer", dynamic_ncols=True)
    ...
    for idx, (layer_name, st) in enumerate(iterator, start=1):
        ...
        if not isinstance(old_layer, nn.Linear):
            raise TypeError(f"Expected nn.Linear at {layer_name}, got {type(old_layer)}")

        target_device = old_layer.weight.device
        target_dtype = old_layer.weight.dtype

        w_cpu = reconstruct_dense_weight_from_alphowl_state_cpu(st, dtype=target_dtype)

        if tuple(w_cpu.shape) != tuple(old_layer.weight.shape):
            raise ValueError(
                f"Weight shape mismatch for {layer_name}: "
                f"checkpoint {tuple(w_cpu.shape)} vs model {tuple(old_layer.weight.shape)}"
            )

        old_layer.weight.data.copy_(w_cpu.to(device=target_device, dtype=target_dtype))
        ...
        if tqdm is None and show_progress:
            ...
            print(
                f"\rApplying alphowl sparse layers: {idx}/{len(items)} "
    ...
    cleanup_cuda()
    print(f"Alpha/OWL sparse layers applied in {format_seconds(now() - t0)}")
    print_cuda_memory("CUDA memory after Alpha/OWL sparse apply")


# =============================================================================
# Checkpoint detection / application
# =============================================================================

def detect_checkpoint_type(ckpt: Dict[str, Any]) -> str:
    if "alphowl_sparse_layers" in ckpt:
        return "alphowl_sparse_layers"
    if "joint_sparsegpt_gptq_layers" in ckpt:
        return "joint_sparsegpt_gptq_layers"
    if "layers" in ckpt:
        return "layers"
    if "hybrid_sparse_layers" in ckpt:
        return "hybrid_sparse_layers"
    raise ValueError(
        "Unknown compressed checkpoint format. Expected one of:\n"
        "  - alphowl_sparse_layers\n"
...
def get_checkpoint_global_meta(ckpt: Dict[str, Any]) -> Dict[str, Any]:
    if "alphowl_sparse_meta" in ckpt:
        return ckpt["alphowl_sparse_meta"]
    if "joint_sparsegpt_gptq_meta" in ckpt:
        return ckpt["joint_sparsegpt_gptq_meta"]
    if "hybrid_sparse_meta" in ckpt:
        return ckpt["hybrid_sparse_meta"]
    if "compression_meta" in ckpt:
        return ckpt["compression_meta"]
    return ckpt


def normalize_joint_layer_state(st: Dict[str, Any]) -> Dict[str, Any]:
...
def optionally_load_checkpoint_model_state(
    model: nn.Module,
    ckpt: Dict[str, Any],
    load_checkpoint_model: bool,
) -> None:
    """
    Loads ckpt["model"] if requested.

    For sparse-only checkpoints this is usually unnecessary if --model_id matches
    the compression model. For full dense-zero checkpoints, this can load all
    pruned weights at once.
    """
    ...
    print("Loading ckpt['model'] state_dict with strict=False...")
    t0 = now()

    missing, unexpected = model.load_state_dict(ckpt["model"], strict=False)

    print(f"ckpt['model'] loaded in {format_seconds(now() - t0)}")
...
@torch.no_grad()
def apply_joint_sparsegpt_gptq_checkpoint(
    model: nn.Module,
    ckpt: Dict[str, Any],
    layers_key: str,
    ...
    """
    Applies older quantized compressed layer format by wrapping selected layers.
    """
    layers = ckpt[layers_key]
    if not isinstance(layers, dict) or len(layers) == 0:
        raise ValueError(f"Checkpoint has empty or invalid ckpt['{layers_key}'].")

    fallback_device = get_model_device(model)
    items = list(layers.items())

    print(f"Applying quantized compressed layers from ckpt['{layers_key}']")
    ...
    iterator = items
    if show_progress and tqdm is not None:
        iterator = tqdm(items, desc="Applying quantized layers", unit="layer", dynamic_ncols=True)

    apply_t0 = now()

    for idx, (layer_name, st_raw) in enumerate(iterator, start=1):
        st = normalize_joint_layer_state(st_raw)
        ...
        if tqdm is None and show_progress:
            ...
            print(
                f"\rApplying quantized layers: {idx}/{len(items)} "
                f"({100.0 * idx / len(items):5.1f}%) "
    ...
    cleanup_cuda()
    print(f"Quantized compressed layers applied in {format_seconds(now() - apply_t0)}")
    print_cuda_memory("CUDA memory after applying quantized checkpoint")


@torch.no_grad()
def apply_hybrid_sparse_checkpoint(
    model: nn.Module,
    ckpt: Dict[str, Any],
    dtype: torch.dtype,
    show_progress: bool,
    ...
    """
    Optional support for your older hybrid_sparse_layers format:
        ckpt["hybrid_sparse_layers"][name]["mask"]
        ckpt["hybrid_sparse_layers"][name]["values"]
        values_format == "kept_1d"
    """
    layers = ckpt.get("hybrid_sparse_layers", None)
    if not isinstance(layers, dict) or len(layers) == 0:
        raise ValueError("Checkpoint does not contain valid ckpt['hybrid_sparse_layers'].")

    items = list(layers.items())
    print(f"Applying hybrid sparse layers: {len(items):,}")

    iterator = items
    if show_progress and tqdm is not None:
        iterator = tqdm(items, desc="Applying hybrid sparse layers", unit="layer", dynamic_ncols=True)
    ...
    for idx, (layer_name, st) in enumerate(iterator, start=1):
        ...
        if mask_packing == "packedbits":
            mask = unpack_bool_mask_rows(st["mask"].cpu().to(torch.uint8), original_cols=cols)
        elif mask_packing == "bool":
            mask = st["mask"].cpu().bool()
        else:
            raise ValueError(f"Unsupported hybrid mask_packing: {mask_packing}")

        values = st["values"].detach().cpu().to(dtype=old_layer.weight.dtype)

        if values.ndim == 2:
            w_cpu = values * mask.to(values.dtype)
        else:
            n_kept = int(mask.sum().item())
            if values.numel() != n_kept:
                raise ValueError(
                    f"Kept values mismatch for {layer_name}: "
                    f"values={values.numel():,}, mask_kept={n_kept:,}"
                )
            w_cpu = torch.zeros((rows, cols), dtype=old_layer.weight.dtype)
            w_cpu[mask] = values

        old_layer.weight.data.copy_(w_cpu.to(device=old_layer.weight.device, dtype=old_layer.weight.dtype))

        del w_cpu, mask, values
        ...
        if tqdm is None and show_progress:
            elapsed = now() - t0
            print(
                f"\rApplying hybrid sparse layers: {idx}/{len(items)} "
                f"({100.0 * idx / len(items):5.1f}%) elapsed={format_seconds(elapsed)}",
                end="",
                flush=True,
            )
    ...
    cleanup_cuda()
    print(f"Hybrid sparse layers applied in {format_seconds(now() - t0)}")
...
def apply_compressed_checkpoint(
    ...
    """
    Universal checkpoint application.
    """
    ...
    layers_key = detect_checkpoint_type(ckpt)

    # Optional: load ckpt["model"] first if user asks.
    # For sparse-only checkpoints, leave this off unless you intentionally saved
    # a full dense-zero state dict.
    optionally_load_checkpoint_model_state(
        model=model,
        ckpt=ckpt,
        load_checkpoint_model=load_checkpoint_model,
    )

    if layers_key == "alphowl_sparse_layers":
        apply_alphowl_sparse_checkpoint(
            model=model,
            ckpt=ckpt,
            dtype=dequant_dtype,
    ...
    elif layers_key in ("joint_sparsegpt_gptq_layers", "layers"):
        apply_joint_sparsegpt_gptq_checkpoint(
            model=model,
            ckpt=ckpt,
    ...
    elif layers_key == "hybrid_sparse_layers":
        apply_hybrid_sparse_checkpoint(
            model=model,
            ckpt=ckpt,
            dtype=dequant_dtype,
            show_progress=show_progress,
        ...
        raise RuntimeError(f"Unhandled checkpoint layers key: {layers_key}")

    model.eval()
    return model, ckpt, layers_key
...
def evaluate_metrics(
    ...
    def process_batch(
        ...
        with ctx:
            ...
            # Do NOT shift logits or labels again.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate HF original or compressed sparse checkpoint perplexity."
    )

    parser.add_argument(
        "--input_file",
        type=str,
        default="data/eval_wikitext103_test_shared_250k.txt",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "For older quantized checkpoints only. Cache dequantized weights. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Usually OFF for sparse-only checkpoints if --model_id matches."
        ),
    )
    ...
    if args.compressed is not None:
        ...
        model, compressed_info, layers_key = apply_compressed_checkpoint(
            model=model,
            compressed_path=args.compressed,
            cache_dequantized=bool(args.cache_dequantized),
            show_progress=show_progress,
            load_checkpoint_model=bool(args.load_checkpoint_model),
            dequant_dtype=dequant_dtype,
        )

        if layers_key == "alphowl_sparse_layers":
            model_variant = "compressed_alphowl_sparsegpt_sparseswaps"
        elif layers_key == "hybrid_sparse_layers":
            model_variant = "compressed_hybrid_sparse_layers"
        else:
    ...
    print(f"Checkpoint key    : {layers_key}")
    ...
    if compressed_info is not None:
        ...
        if layers_key in ("joint_sparsegpt_gptq_layers", "layers"):
        ...
        target_sparse = (
            global_meta.get("target_sparsity", None)
            if global_meta.get("target_sparsity", None) is not None
            else global_meta.get("sparsity", compressed_info.get("sparsity", "unknown"))
        )
        ...
        print(f"Target sparsity   : {target_sparse}")
        print(f"Actual sparsity   : {100.0 * float(actual_sparsity):.2f}%")
        print(f"Compressed layers : {len(compressed_info.get(layers_key, {})) if layers_key else 0}")
