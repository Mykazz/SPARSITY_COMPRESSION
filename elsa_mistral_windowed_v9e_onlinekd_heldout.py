#!/usr/bin/env python3
"""
v9e — Patch of v9d adding two quality upgrades, both OFF by default (so the
      default command reproduces the v9d/17.07 recipe exactly):

  1. ON-THE-FLY FULL-VOCAB KD  (--kd_online)
     Instead of distilling from the cached top-K teacher (which renormalizes
     over the top-K, ignores student probability mass that leaks outside it,
     and freezes the teacher at the build temperature), keep a dense teacher
     resident on the GPU and compute EXACT full-vocabulary KL each step, with
     temperature applied consistently to teacher and student (true Hinton KD).
     Removes the top-K and temperature approximations entirely. Uses the spare
     VRAM on a 48 GB card (teacher ~14 GB + student). Recommended at
     --max_seq_len 1024 for headroom; at 2048 watch the first-window CUDA line.
     When --kd_online is set, the top-K cache is neither built nor used.

  2. HELD-OUT CALIBRATION PROBE  (--heldout_probe_rows N)
     Reserve the last N rows of the calibration tensor as a NEVER-TRAINED
     held-out set (excluded from training, the KD cache, and Wanda stats).
     Probe loss/ppl on it at init, after each ADMM pass, and at the end — an
     honest generalization signal, unlike the existing train-crop probe whose
     low ppl was pure memorization (the 4.7-vs-18 illusion). Use it to choose
     hyper-parameters/passes without spending eval-only runs. 0 disables.

  Everything from v9d (local reconstruction, AlphaPruning, AWQ/hidden-KD fixes,
  cleanup_max_seq_len) is preserved and unchanged when these flags are off.
...
def teacher_logits_for_batch(
    ...
    """v9e: one dense-teacher forward (no grad) for on-the-fly full-vocab KD."""
...
def compute_kd_full_loss(
    student_logits: torch.Tensor,    # [B, T, V]
    teacher_logits: torch.Tensor,    # [B, T, V] (same batch/seq as student)
    temperature: float = 1.0,
) -> torch.Tensor:
    """v9e: EXACT full-vocabulary KD.

    KL(teacher_T || student_T) over the whole vocabulary, with temperature
    applied consistently to BOTH distributions (true Hinton distillation).
    Both are sliced [:, :-1, :] for next-token alignment.

    This removes the two approximations of the cached top-K path:
      * top-K truncation + renormalization (which ignores student mass that
        leaks outside the teacher's top-K), and
      * the temperature mismatch (the cache freezes the teacher at build-T,
        so only the student was being softened).
    Costs one extra (no-grad) dense-teacher forward per microbatch and a few
    hundred MB of FP32 transient at V=32000.
    """
    if student_logits.dim() != 3 or teacher_logits.dim() != 3:
        raise ValueError("logits must be [B, T, V]")
    T = max(1.0e-6, float(temperature))
    L = min(int(student_logits.size(1)), int(teacher_logits.size(1)))
    if L <= 1:
        return student_logits.new_zeros(())
    sl = student_logits[:, :L - 1, :].float() / T
    tl = teacher_logits[:, :L - 1, :].float() / T
    t_logp = F.log_softmax(tl, dim=-1)
    s_logp = F.log_softmax(sl, dim=-1)
    t_p = t_logp.exp()
        ...
        for c_step in range(int(args.cleanup_steps)):
            ...
            try:
                for accum_idx in range(accum_steps):
                    ...
                    need_logits = kd_alpha > 0.0 and (kd_cache is not None or kd_teacher is not None)
                    ...
                    kd_loss_i = None
                    if need_logits and full_logits is not None and kd_teacher is not None:
                        # v9e: exact full-vocab KD from the resident dense teacher.
                        try:
                            t_logits = teacher_logits_for_batch(
                                teacher=kd_teacher, input_ids=batch,
                                amp_dtype=model_dtype, device=main_device,
                                autocast_enabled=not bool(args.no_autocast),
                            )
                            kd_loss_i = compute_kd_full_loss(
                            ...
                            print(f"  [warn] cleanup online KD failed: {exc}")
                            kd_loss_i = None
                    elif need_logits and full_logits is not None:
                        try:
...
def run(args: argparse.Namespace) -> None:
    ...
    # v9e: reserve a never-trained held-out slice for an honest generalization probe.
    heldout_tokens: Optional[torch.Tensor] = None
    _heldout_rows = int(getattr(args, "heldout_probe_rows", 0))
    if _heldout_rows > 0:
        if _heldout_rows >= int(calib_tokens.size(0)):
            print(f"[heldout] requested {_heldout_rows} held-out rows >= calib rows "
                  f"{calib_tokens.size(0)}; disabling held-out probe.")
        else:
            heldout_tokens = calib_tokens[-_heldout_rows:].contiguous()
            calib_tokens = calib_tokens[:-_heldout_rows].contiguous()
            print(f"[heldout] reserved last {_heldout_rows} rows as a never-trained held-out "
                  f"probe set; training rows now {calib_tokens.size(0)}.")

    kd_cache: Optional[KDTopKCache] = None
    kd_teacher: Optional[nn.Module] = None
    if float(args.kd_alpha) > 0.0 and bool(getattr(args, "kd_online", False)):
        # v9e: on-the-fly full-vocab KD — keep a resident dense teacher, no cache.
        print("\n[KD online] loading a resident dense teacher for full-vocab KD "
              "(top-K cache disabled)...")
        ...
        if hasattr(kd_teacher, "config"):
            kd_teacher.config.use_cache = False
        kd_teacher.to(main_device)
        kd_teacher.eval()
        for _tp in kd_teacher.parameters():
            _tp.requires_grad_(False)
        print(f"[KD online] dense teacher resident. {cuda_mem()}")
        print("[KD online] NOTE: teacher + student both live on GPU; if you OOM at "
              "--max_seq_len 2048, drop to 1024.")
    elif float(args.kd_alpha) > 0.0:
    ...
    try:
        for pass_idx in range(int(args.admm_passes)):
                        ...
                        for accum_idx in range(accum_steps):
                            ...
                            if need_logits and full_logits is not None and kd_teacher is not None:
                                ...
                                try:
                                    ...
                                    print(f"  [warn] online KD failed: {exc}; skipping KD this step.")
                                    kd_loss_i = None
                            elif need_logits and full_logits is not None:
                                try:
            ...
            # v9e: honest generalization signal on the never-trained held-out rows.
            if heldout_tokens is not None and int(args.pass_probe_batches) > 0:
                ho_probe = quick_calibration_loss(
                    model=model,
                    tokens=heldout_tokens,
                    batch_size=int(args.batch_size),
                    max_seq_len=int(args.max_seq_len),
                    batches=int(args.pass_probe_batches),
                    seed=int(args.seed) + 5000 + int(pass_idx),
                    device=main_device,
                    amp_dtype=model_dtype,
                    crop_mode=str(args.crop_mode),
                )
                print(
                    f"Pass {pass_idx + 1} HELD-OUT probe: "
                    f"loss={ho_probe['probe_loss']:.6f} ppl~{ho_probe['probe_ppl']:.3f} "
                    f"tokens={ho_probe['probe_tokens']:.0f}  (never-trained rows)"
                )
                store.save_json(f"pass_{pass_idx + 1}_heldout_probe.json", ho_probe)
    ...
    # v9e: final honest generalization probe on the never-trained held-out rows.
    final_heldout_probe = None
    if heldout_tokens is not None and int(args.final_probe_batches) > 0:
        final_heldout_probe = quick_calibration_loss(
            model=model,
            tokens=heldout_tokens,
            batch_size=int(args.batch_size),
            max_seq_len=int(args.max_seq_len),
        ...
        print(
            f"Final HELD-OUT probe: loss={final_heldout_probe['probe_loss']:.6f} "
            f"ppl~{final_heldout_probe['probe_ppl']:.3f} "
            f"tokens={final_heldout_probe['probe_tokens']:.0f}  (never-trained rows; "
            f"this tracks eval ppl far better than the calibration probe)"
        )
        try:
            store.save_json("final_heldout_probe.json", final_heldout_probe)
        except Exception:
            pass
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # ============================================================
    # v9e: on-the-fly full-vocab KD + held-out generalization probe
    # ============================================================
    parser.add_argument("--kd_online", action="store_true",
                        help="v9e: distill from a RESIDENT dense teacher with EXACT full-vocabulary KL "
                             "every step (temperature applied to both teacher and student), instead of "
                             "the cached top-K teacher. Removes the top-K renormalization and "
                             "temperature-mismatch approximations. Requires kd_alpha>0. Keeps teacher "
                             "(~14GB) + student on the GPU; comfortable at --max_seq_len 1024 on 48GB, "
                             "tight at 2048. When set, the top-K cache is neither built nor used.")
    parser.add_argument("--heldout_probe_rows", type=int, default=0,
                        help="v9e: reserve the last N rows of the calibration tensor as a NEVER-TRAINED "
                             "held-out set (excluded from training, KD cache, and Wanda stats). Probe "
                             "loss/ppl on it at init/each pass/final for an honest generalization signal. "
                             "0 disables. Try 128-256.")
