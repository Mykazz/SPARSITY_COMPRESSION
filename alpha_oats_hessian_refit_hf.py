...
Fixed version:
  - Always clamps Hessian activations before X^T X.
  - Sanitizes local Hessian contributions instead of skipping whole batches.
  - Clamps propagated hidden cache to prevent later-block explosion.
  - Adds diagonal Hessian fallback if a layer still gets zero valid samples.
  - Saves sparse-only checkpoints by default to avoid huge torch.save failures.

Method:
  1. Dense calibration prepass:
       - OWL-style layer outlier ratios
       - AlphaPruning-style PL_Alpha_Hill weight-spectrum metrics
       - non-uniform layer/block sparsity allocation

  2. Blockwise pruning:
       - collect H = 2 X^T X for selected Linear layers
       - row-balanced SparseGPT warm-start mask
       - SparseSwaps 1-swap mask refinement
       - fixed-mask SparseGPT/OBS reconstruction
       - propagate compressed block outputs

Example:

python mistral_alphowl_sparsegpt_swaps_fixed.py \
  --model_id mistralai/Mistral-7B-Instruct-v0.3 \
  --calib data/calib_wikitext103_train_128x2048_mistral.pt \
  --out compressed/test_mlp_s70_alphowl_swaps_sparseonly.pt \
...
def run_block_to_cache(
    ...
    for i in range(0, n, batch_size):
        ...
        # CRUCIAL FIX:
        # Always clamp propagated hidden states, not only when non-finite.
...
class InputSecondMomentCollector:
    ...
    def _hook(self, module: nn.Module, inputs: Tuple[torch.Tensor, ...]) -> None:
        ...
        # CRUCIAL FIX:
        # Always clamp, because finite huge values can still overflow X^T X.
...
def collect_allocation_stats_prepass(
    ...
    for block_idx, block in enumerate(decoder_layers):
        ...
        for name, layer in items:
            collectors[name] = InputSecondMomentCollector(layer, name, activation_clamp=activation_clamp)
...
class HessianCollector:
    ...
    Fixed behavior:
      - always clamps X before matmul
      - if local_H has rare non-finite entries, sanitize it and keep contribution
      - never drops the whole local contribution unless shape is invalid
    """
    ...
    def _hook(self, module: nn.Module, inputs: Tuple[torch.Tensor, ...]) -> None:
        ...
        # CRUCIAL FIX:
        # Always sanitize and clamp before X^T X.
        x = sanitize_tensor(x, clamp_abs=self.activation_clamp)

        x = x.to(device=self.hessian_device, dtype=self.dtype)

        # Matmul in fp32/fp64 according to hessian_dtype.
        ...
        if not torch.isfinite(local_H).all():
            ...
            if not self.warned_nonfinite_hessian:
                print(
                    ...
                    f"{bad:,}/{total:,}; sanitizing local contribution instead of skipping."
                )
...
# =============================================================================
# Mask generation and SparseSwaps
# =============================================================================
...
def sparse_swaps_refine_mask(
    ...
    W = sanitize_tensor(W.float(), clamp_abs=1.0e8)
    G = sanitize_tensor(G.float(), clamp_abs=1.0e30)
    ...
    C = sanitize_tensor(residual_weights.matmul(G), clamp_abs=1.0e30)
    ...
    for i in range(rows):
        ...
        for _it in range(max_iters):
            ...
            c.add_(G[:, u], alpha=wu_scalar)
            c.add_(G[:, p], alpha=-wp_scalar)
...
def make_diagonal_hessian_fallback(
    ...
    H = torch.eye(cols, device=device, dtype=dtype) * float(diag_value)
    return H
        ...
        for local_idx, (layer_name, layer) in enumerate(items, start=1):
            ...
            if nsamples <= 0:
                if bool(args.allow_hessian_diag_fallback):
                    print(
                        f" [warn] No Hessian samples for {layer_name}. "
                        f"Using diagonal fallback H = I. This layer will use a weaker mask."
                    )
...
def build_meta(
    ...
    meta: Dict[str, Any] = {
        "method": "alphowl_rowbalanced_sparsegpt_sparseswaps_fixedmask_obs_no_quant_fixed_hessian",
        "model_id": str(args.model_id),
...
def main() -> None:
    ...
    parser.add_argument("--activation_clamp", type=float, default=1000.0)
    parser.add_argument("--hidden_clamp", type=float, default=10000.0)
    ...
    print("=" * 100)
    print("Mistral Alpha/OWL + row-balanced SparseGPT + SparseSwaps + fixed-mask OBS [FIXED]")
    print("=" * 100)
