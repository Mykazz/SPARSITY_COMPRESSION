...
def run_cleanup_phase(
    ...
    # v9b: cleanup may use a shorter sequence than ADMM to halve per-step activation memory.
    ...
    for win_idx, active_names in enumerate(windows):
        ...
        # v9c-fix: do the fp16->fp32 conversion on CPU (not GPU). Previously
        # `.detach().float().cpu()` allocated a 224MB fp32 tensor on GPU first
        # for a Mistral down_proj — which crashes the whole run after a window
        # has OOM-skipped all cleanup steps (no GPU headroom left for the
        # transient). Now we move to CPU first (fp16 stays ~112MB on CPU) and
        # convert to fp32 on CPU after. Also wrapped in try/except so even a
        # late-window OOM doesn't kill the run.
        try:
            with torch.no_grad():
                for name in active_names:
                    w = get_module_by_name(model, name).weight.detach().cpu().float()
            ...
            msg = str(exc).lower()
            if "out of memory" in msg or isinstance(exc, torch.cuda.OutOfMemoryError):
                print(f"  [warn] cleanup window {win_idx + 1} weight-save hit OOM ({exc}); "
                      f"keeping prior z (post-ADMM) for these layers and continuing.")
                for _ in range(3):
                    gc.collect()
                if torch.cuda.is_available():
                    torch.cuda.synchronize()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--cleanup_grad_accum_steps", type=int, default=-1,
                        help="v9b: gradient-accumulation steps used during cleanup. -1 means inherit "
                             "--grad_accum_steps. Lowering this reduces peak activation memory PER cleanup "
                             "step at the cost of noisier gradients; useful when cleanup OOMs but ADMM does not.")
    ...
    parser.add_argument("--cleanup_max_seq_len", type=int, default=-1,
                        help="v9b: max sequence length used by the cleanup-phase token loader. "
                             "-1 means inherit --max_seq_len. Lower this (e.g. 512) to halve per-step activation "
                             "memory if cleanup OOMs after a few windows from allocator fragmentation drift. "
                             "Cleanup at seq 512 still gives most of the perplexity benefit because the cleanup "
                             "phase optimizes locally on the frozen mask — long-range context is less critical "
                             "than during ADMM. Recommended on 24GB GPUs when --cleanup_steps > 32.")
    parser.add_argument("--cleanup_disable_output_scale_skipped_windows", action="store_true",
                        default=True,
                        help="v9b: ensure that windows skipped due to OOM do not leave dangling forward hooks "
                             "from learn_output_scale registration (already removed at window-end on success; "
                             "this flag adds redundant cleanup on the OOM exception path).")
